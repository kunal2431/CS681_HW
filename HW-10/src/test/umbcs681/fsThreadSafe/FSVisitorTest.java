...
import umbcs681.fsThreadSafe.util.*;
...
public class FSVisitorTest{
    ...
    public void verify_FileCrawlingVisitor(){
        ...
        repo.accept(visitor);
        LinkedList<RunnableFile> actual =  visitor.getFiles();
    ...
    public void verify_StreamFileCrawlingVisitor(){
        ...
        FileCrawlingVisitor visitor = new FileCrawlingVisitor();
        repo.accept(visitor);
        long count = visitor.files().filter((RunnableFile file)->file.getcreationTime().isAfter(time)).filter(file -> file.getName().endsWith(".java")).count();
    ...
    public void verify_StreamGroup(){
        FileCrawlingVisitor visitor = new FileCrawlingVisitor();
        repo.accept(visitor);
        Map<String, List<RunnableFile>> grpFiles = visitor.files().collect(groupingBy( (RunnableFile file)-> { if(file.getName().endsWith(".java")) { return ".java";} else{return ".md";}}));
        ...
        grpFiles.forEach((group, files)->{
            IntSummaryStatistics stats = files.stream().collect(summarizingInt((RunnableFile file)-> file.getSize()));
